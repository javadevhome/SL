...
<%@include file="/WEB-INF/pages/commen/header.jsp"%>
...
<div>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <c:forEach items="${roleList}" var="role">
							<tr>
								...
								isstart="${role.isStart}" roleid="${role.id}" <c:if test="${role.isStart == 1}">checked="true"</c:if>/>
								</td>
	...
	<div class="modal hide fade" id="addRoleDiv">
	 ...
	 <!-- 显示存在的用户loginCode -->
	 	<div class="modal hide fade" id="showLoginCode">
			<div class="modal-header">
				<button type="button" id="addRoleClose" class="close" data-dismiss="modal">×</button>
				<h4>系统中有用户被授权该角色，不能被删除！账号如下</h4>
			</div>
			<div class="modal-body">
				<ul class="topul" id="userCode">
				</ul>
				<div class="clear"></div>
			</div>
...
<%@include file="/WEB-INF/pages/commen/footer.jsp"%>
