...
$(".modifyIsStart").click(function() {
	...
	$.ajax({
		...
		type : 'POST',
		data : {
			role : JSON.stringify(roleIstart)
		},
		dataType : 'html',
		timeout : 1000,
		error : function() {
				...
				if (data.result != "" && "success" == data.result) {
					...
					if (data.result != null && data.result != "") {
						...
						for(var i=0;i<data.result.length;i++){
							opt+="<li><b>"+data.result[i].loginCode+"</b></li>";
						}
						$("#userCode").append(opt);
						e.preventDefault();
						$('#showLoginCode').modal('show');
						//alert("系统中有用户被授权该角色，不能被删除！用户账号如下：" + result.loginCode);
					}
